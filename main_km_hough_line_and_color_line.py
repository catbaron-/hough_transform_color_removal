...
# apply KM when cluster RGB points to different colors
...
class ColorRemover:
    ...
    def __init__(self, img_name):
        ...
        self.points_of_color = {}
        self.pixels_of_color = {}
    ...
    def cluster_points_to_color_line(self):
        ...
        def cluster_points_label(label):
            pixels = self.pixels_of_color[label]
            ...
            for pixel in pixels:
                ...
                for line in self.color_lines[label]:
                    if self.img.get_bgr_norm(line[0]) < norm < self.img.get_bgr_norm(line[1]):
                        ...
                        break
        for label in self.points_of_color:
    ...
    def show_lines_points(self, lines, points, title="lines & points"):
        ...
        # self._img.draw_hemi(ax, HEMISPHERE_NUM, self.slice_width)
        plt.show()
    ...
    def cluster_colors(self):
        fg_points = self.img.fg_points.keys()
        descriptors = np.array(map(lambda point: self.hough_lines_of_point[point], fg_points))
        fg_points = np.array(fg_points)
        km = KMeans(n_clusters=NUM_OF_COLOR, max_iter=600)
        labels = km.fit_predict(descriptors)
        for label in labels:
            self.points_of_color[label] = map(tuple, fg_points[labels==label])
            pixels = []
            for point in self.points_of_color[label]:
                pixels += self.img.points_pixels[tuple(point)]
            self.pixels_of_color[label] = map(tuple, pixels)
    ...
    def main_process_of_color_line_sphere_hough(self):
        ...
        self.generate_color_points_with_label(self.points_of_color)
        print "done"
...
if __name__ == "__main__":
    files = ["_img/man.bmp", "_img/bird.bmp", "_img/nasu.bmp"]
    # files = ["_img/bird.bmp"]
    for file in files:
